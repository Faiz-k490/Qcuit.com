...
function circuitReducer(state: CircuitState, action: CircuitAction): CircuitState {
  switch (action.type) {
    ...
    case 'LOAD_CIRCUIT': {
      const numQubits = Math.max(1, Math.min(20, Number(action.payload.numQubits) || initialState.numQubits));
      const numClassical = Math.max(1, Math.min(20, Number(action.payload.numClassical) || numQubits));
      const numTimesteps = Math.max(1, Math.min(200, Number(action.payload.numTimesteps) || initialState.numTimesteps));

      const normalizedGates: Record<string, GateInstance> = {};
      const inputGates = action.payload.gates && typeof action.payload.gates === 'object' ? action.payload.gates : {};
      Object.entries(inputGates).forEach(([key, gate]) => {
        if (!gate) return;
        const g: GateInstance = {
          ...(gate as GateInstance),
          id: (gate as GateInstance).id || uuidv4(),
          qubit: Number((gate as GateInstance).qubit) || 0,
          timestep: Number((gate as GateInstance).timestep) || 0,
        };
        normalizedGates[key] = g;
      });

      const normalizedMulti = Array.isArray(action.payload.multiQubitGates)
        ? action.payload.multiQubitGates.map((g) => ({
            ...(g as MultiQubitGateInstance),
            id: (g as MultiQubitGateInstance).id || uuidv4(),
            timestep: Number((g as MultiQubitGateInstance).timestep) || 0,
            controls: Array.isArray((g as MultiQubitGateInstance).controls) ? (g as MultiQubitGateInstance).controls : [],
            targets: Array.isArray((g as MultiQubitGateInstance).targets) ? (g as MultiQubitGateInstance).targets : [],
          }))
        : [];

      const normalizedMeasurements = Array.isArray(action.payload.measurements)
        ? action.payload.measurements.map((m) => ({
            ...(m as Measurement),
            id: (m as Measurement).id || uuidv4(),
            qubit: Number((m as Measurement).qubit) || 0,
            classicalBit: Number((m as Measurement).classicalBit) || 0,
            timestep: Number((m as Measurement).timestep) || 0,
          }))
        : [];
      ...
      const normalized: CircuitState = {
        ...
        multiQubitGates: normalizedMulti,
        measurements: normalizedMeasurements,
        noiseLevel: typeof action.payload.noiseLevel === 'number' ? action.payload.noiseLevel : initialState.noiseLevel,
      };
...
export function CircuitProvider({ children }: { children: ReactNode }) {
  ...
  const loadCircuit = useCallback((circuit: CircuitState) => {
    const numQubits = Math.max(1, Math.min(20, Number(circuit.numQubits) || initialState.numQubits));
    const numClassical = Math.max(1, Math.min(20, Number(circuit.numClassical) || numQubits));
    const numTimesteps = Math.max(1, Math.min(200, Number(circuit.numTimesteps) || initialState.numTimesteps));

    const normalizedGates: Record<string, GateInstance> = {};
    const inputGates = circuit.gates && typeof circuit.gates === 'object' ? circuit.gates : {};
    Object.entries(inputGates).forEach(([key, gate]) => {
      if (!gate) return;
      const g: GateInstance = {
        ...(gate as GateInstance),
    ...
    const normalizedMulti = Array.isArray(circuit.multiQubitGates)
      ? circuit.multiQubitGates.map((g) => ({
          ...(g as MultiQubitGateInstance),
          id: (g as MultiQubitGateInstance).id || uuidv4(),
          timestep: Number((g as MultiQubitGateInstance).timestep) || 0,
          controls: Array.isArray((g as MultiQubitGateInstance).controls) ? (g as MultiQubitGateInstance).controls : [],
    ...
    const normalizedMeasurements = Array.isArray(circuit.measurements)
    ...
    const normalized: CircuitState = {
      ...
      multiQubitGates: normalizedMulti,
      measurements: normalizedMeasurements,
      noiseLevel: typeof circuit.noiseLevel === 'number' ? circuit.noiseLevel : initialState.noiseLevel,
    };

    setResults(null);
    dispatch({ type: 'LOAD_CIRCUIT', payload: normalized });
  }, []);
