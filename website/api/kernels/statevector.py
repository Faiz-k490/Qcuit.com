...
class StatevectorKernel(ISimulationKernel):
    ...
    def _apply_single_qubit_gate(self, gate: np.ndarray, target: int) -> None:
        """Apply gate using vectorized NumPy — updates only affected amplitude pairs.

        Instead of a Python for-loop over 2^n indices, we build index arrays
        for the paired amplitudes (target-bit == 0 and target-bit == 1) and
        apply the 2×2 gate as a single vectorized operation.
        """
        n = 2 ** self.num_qubits
        target_mask = 1 << target

        # Build an array of ALL indices [0, 1, ..., n-1], then select those
        # whose target bit is 0.  The partner index (target bit = 1) is
        # obtained by bitwise-OR with the target mask.
        all_indices = np.arange(n, dtype=np.intp)
        idx0 = all_indices[(all_indices & target_mask) == 0]  # target bit == 0
        idx1 = idx0 | target_mask                              # target bit == 1

        # Extract amplitude pairs and apply the 2×2 gate in one shot.
        a0 = self.state[idx0].copy()
        a1 = self.state[idx1].copy()
        self.state[idx0] = gate[0, 0] * a0 + gate[0, 1] * a1
        self.state[idx1] = gate[1, 0] * a0 + gate[1, 1] * a1
